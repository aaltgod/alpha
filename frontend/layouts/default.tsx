			...
			<div class={styles.layout}>
				<header class="sticky top-0 flex h-16 items-center gap-4 border-b bg-background px-4 md:px-6">
					<NavigationMenu class="hidden flex-col gap-6 text-lg font-medium md:flex md:flex-row md:items-center md:gap-5 md:text-sm lg:gap-6">
						...
						{menuItems.map((item) => (
							  <NavigationMenuItem class={$router.currentRoute.value.path == item.to ? "text-foreground transition-colors hover:text-foreground" : "text-muted-foreground transition-colors hover:text-foreground"}>
								<NuxtLink to={item.to}>
									<NavigationMenuLink>
										{item.label}
									</NavigationMenuLink> 
